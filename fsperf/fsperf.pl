...
# Some events uses __perf_task() to set target task, instead of
# current.  When __perf_task() was used and event had specified pid,
# the event might be added to for each event fd.  (e.g. if without
# "-a", perf (v4.8) calls perf_event_open() with target pid for each
# cpus.)
#
# If this was happened, same events was generated to for each fd, and
# perf sees duplicated same events. To fix it, this function try to
# detect duplicated events. (This bug should be kernel bug, and should
# be fixed though)
#
# At kernel v4.8, __perf_task() users (duplicated events) are:
#    sched_wakeup_template
#    sched_stat_template
my %perf_dup_event;
sub perf_is_duplicate(@)
{
    my $perf_task = shift;
    ...
    # New event?
    if (!exists($perf_dup_event{$event_name}) or
	!exists($perf_dup_event{$event_name}{$common_pid})) {
	goto new_event;
    }
    # Check if duplicated event
    my $prev_args = $perf_dup_event{$event_name}{$common_pid};
    foreach my $idx (0 .. $#args) {
	if ($prev_args->[$idx] ne $args[$idx]) {
	    goto new_event;
	}
    }
...
sub sched::sched_stat_blocked
{
	...
	$common_pid, $common_comm, $common_callchain,
	$comm, $pid, $delay) = @_;
    return if perf_is_duplicate($pid, @_);
...
my (%x86_64_syscalltbl);
my %syscall_arch = ("x86_64" => \%x86_64_syscalltbl);

sub syscall_name($)
{
    my $id = shift;
    my $syscall_tbl = $syscall_arch{$perf_arch};

    if ($syscall_tbl) {
	if (exists($syscall_tbl->{$id})) {
	    return $syscall_tbl->{$id};
	}
    }
    return "$id";
}
...
sub syscall_id($)
{
    ...
    if ($syscall_tbl) {
	foreach my $id (keys(%{$syscall_tbl})) {
	    if ($syscall_tbl->{$id} eq $name) {
		return $id;
	    }
	}
    }
...
# Generated from linux-4.8/arch/x86/include/generated/uapi/asm/unistd_64.h
%x86_64_syscalltbl = (
    0 => "read",
    1 => "write",
    2 => "open",
